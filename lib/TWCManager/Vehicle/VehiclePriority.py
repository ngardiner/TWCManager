    ...
    def __getattr__(self, name):
        def method(*args, **kwargs):
            ...
            while priority > 0:
                ...
                logger.debug(f"VehiclePriority: Trying {module_name} for {name} (priority {priority}, {max_attempts} attempts)")
                ...
                for attempt in range(max_attempts):
                    try:
                        ...
                        if hasattr(module_ref, name):
                            if attempt > 0:
                                logger.debug(f"VehiclePriority: Retry {attempt}/{retries} for {module_name}.{name}()")
                            ...
                            if ret:
                                logger.debug(f"VehiclePriority: {module_name}.{name}() succeeded on attempt {attempt + 1}")
                                ...
                                if attempt < retries:
                                    logger.debug(f"VehiclePriority: {module_name}.{name}() failed, retrying ({attempt + 1}/{retries})")
                                else:
                                    logger.debug(f"VehiclePriority: {module_name}.{name}() failed after {max_attempts} attempts, trying next module")
                        ...
                        if attempt < retries:
                            logger.debug(f"VehiclePriority: Exception in {module_name}.{name}() on attempt {attempt + 1}: {str(e)}, retrying")
                        else:
                            logger.debug(f"VehiclePriority: Exception in {module_name}.{name}() after {max_attempts} attempts: {str(e)}, trying next module")
                            self.master.stats["moduleFailures"][module_name] = (self.master.stats["moduleFailures"].get(module_name, 0) + 1)
                        continue
