...
class PVPCesPricing:
    ...
    def update(self):
        ...
        if self.lastFetch == 0 or (now.hour < self.lastFetch.hour):
            # Cache not fetched or was fetched yesterday. Fetch values from API.
            ini = (
                str(now.year)
                + "-"
                + str(now.month)
            ...
            url = (
                "https://api.esios.ree.es/indicators/1014?start_date="
                + ini
                + "&end_date="
                + end
            )
    ...
    def getCheapestStartHour(self, numHours, ini, end):
        ...
        if self.todayImportPrice:
            try:
                if end < ini:
                    # If the scheduled hours are between days we consider hours going from 0 to 47
                ...
                while i <= (end - numHours):
                    ...
                    while j < numHours:
                        price = float(
                            self.todayImportPrice["indicator"]["values"][i + j]["value"]
                        )
                        priceH = priceH + price
                        j = j + 1
                    if priceH < minPrice:
                ...
                self.master.debugLog(
                    4,
                    "$PVPCes",
                    "Exception during cheaper pricing analysis",
                )
